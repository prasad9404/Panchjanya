...
export default function TempleArchitecture() {
    ...
    const handleShare = async () => {
        ...
        const shareData = {
            title: getTranslatedValue(temple.name, langCode),
            text: t('temple.shareMessage', { name: getTranslatedValue(temple.name, langCode) }),
                                    ...
                                    <div className="space-y-3 pt-4">
                                        {abbreviationItems.map((item, index) => (
                                            <div key={item.id || index} className="flex items-start gap-3 text-sm text-slate-700 pb-2 border-b border-gray-100 last:border-0">
                                                ...
                                                <span>{getTranslatedValue(item.description, langCode)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </DialogContent>
